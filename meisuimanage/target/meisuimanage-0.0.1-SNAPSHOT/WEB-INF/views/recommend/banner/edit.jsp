									...
									<fieldset class="form-horizontal">
										<form id="tab" action="${BASE_PATH}/recommend${name}/save"
											...
											<div class="form-group">
												...
												<div class="col-sm-5">
													<select name="style" class="form-control">
														<option value="0"<c:if test="${recommend.style==0}"> selected="selected"</c:if>>广场banner</option>
														<option value="2"<c:if test="${recommend.style==2}"> selected="selected"</c:if>>房间广告</option>
														<option value="4"<c:if test="${recommend.style==4}"> selected="selected"</c:if>>用户信息</option>
													</select>
												</div>
											</div>
											<div class="form-group">
												...
												<div class="btn-group col-sm-6">
													<div class="fl">
														...
														<img src="${uploadUrl}${recommend.pic}" class="wd200 mt10" id="image" />
													</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-5">
													<input type="text" name="anchor_uid" class="form-control" value="${recommend.anchor_uid}">
												</div>
											</div>
											<div class="form-group">
											...
											<div class="form-group">
												...
												<div class="col-sm-5">
													<input type="text" class="form-control" name="position" value="${recommend.position}">
												</div>
											</div>
											<div class="form-group">
											...
											<div class="form-group">
												...
												<div class="col-sm-10">
													<div class="radio radio-info radio-inline">
														<input type="radio" value="1" name="is_share" disabled="disabled"<c:if test="${recommend.is_share==1}"> checked="checked"</c:if>/> <label for="inlineRadio3">&nbsp;是</label>
													</div>
													<div class="radio radio-inline">
														<input type="radio" value="0" class="ml15"
															name="is_share" disabled="disabled"<c:if test="${recommend.is_share==0}"> checked="checked"</c:if>/> <label for="inlineRadio4">&nbsp;否</label>
													</div>
												</div>
											</div>											
											<div class="form-group">
												...
												<div class="col-sm-5">
													<input type="text" class="form-control" name="share_title" value="${recommend.share_title}">
												</div>
											</div>											
											<div class="form-group">
												<label class="col-sm-2 control-label">内容：</label>
												<div class="col-sm-5">
													<input type="text" class="form-control" name="share_content" value="${recommend.share_content}">
												</div>
											</div>
											<div class="form-group">
												...
												<div class="btn-group col-sm-6">
													<div class="fl">
														<input name="upload" id="fileInput2" type="file" /><input
															type="hidden" id="imageHidden2" name="share_weixin_pic" value="${recommend.share_weixin_pic}"/>
														<div id="singleDiv2"></div>
														<img src="${uploadUrl}${recommend.share_weixin_pic}" class="<c:if test="${empty recommend.share_weixin_pic }">hide </c:if>wd200 mt10" id="image2" />
													</div>
													<span class="col-sm-5 help-block m-b-none text-danger"
														style="color: blue">&nbsp;&nbsp;&nbsp;图片大小：</span>
												</div>
												...
												<div class="btn-group col-sm-6">
													<div class="fl">
														<input name="upload" id="fileInput3" type="file" /><input
															type="hidden" id="imageHidden3" name="share_weibo_pic" value="${recommend.share_weibo_pic}"/>
														<div id="singleDiv3"></div>
														<img src="${uploadUrl}${recommend.share_weibo_pic}" class="<c:if test="${empty recommend.share_weibo_pic }">hide </c:if>wd200 mt10" id="image3" />
													</div>
													<span class="col-sm-5 help-block m-b-none text-danger"
														style="color: blue">&nbsp;&nbsp;&nbsp;图片大小：</span>
												</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-5">
													<label class="checkbox-inline i-checks"> <input
														type="checkbox" name="platform" value="10"<c:if test="${recommend.platform==10||recommend.platform==00}"> checked="checked"</c:if>>&nbsp;&nbsp;&nbsp;iOS
											...
											<div class="form-group">
												...
												<div class="col-sm-5">
													<select name="channel" class="form-control"
														id="channel">
														<option value="" <c:if test="${recommend.channel==''}"> selected="selected"</c:if>>美岁</option>
														<option value="0101" <c:if test="${recommend.channel=='0101'}"> selected="selected"</c:if>>梦幻直播</option> 
														<option value="0202" <c:if test="${recommend.channel=='0202'}"> selected="selected"</c:if>>嗨兔</option>
													</select>
												</div>
											</div>										
											<div class="form-group">
...
<script type="text/javascript">
	...
	$(function(){	
		...
		$("select[name=jumpstyle]").find("option[value='${recommend.jumpstyle}']").attr("selected",true);
		 if($("select[name=jumpstyle]").val()=='fm://inside')
		{
			$("input[name=is_share]").removeAttr("disabled");
			//$("input[name=is_share][value=0]").prop("checked",false);
			//$("input[name=is_share][value=1]").prop("checked",true);
		}
		$("select[name=jumpstyle]").change(function(){
			if($(this).val()!='fm://inside')
			{
				$("input[name=is_share]").attr("disabled","disabled");
				$("input[name=is_share][value=0]").prop("checked",true);
				$("input[name=is_share][value=1]").prop("checked",false);
			}
			else
			{
				$("input[name=is_share]").removeAttr("disabled");
