								...
								<div class="panel-body">
									<fieldset class="form-horizontal">
	                                        ...
	                                        <div class="col-sm-5">
	                                            <select class="form-control" name="f_id">
	                                               <option value="0">一级菜单</option>
													<c:forEach items="${_menuList}" var="m">
														<c:if test="${m.id>0}">
														<option value="${m.id}"<c:if test="${menu.f_id==m.id}"> selected="selected"</c:if>>${m.name}</option>
														</c:if>
													</c:forEach>
	                                            </select>
	                                        </div>
										...
										<div class="form-group"><label class="col-sm-2 control-label">是否有子菜单：</label>
	                                        <div class="col-sm-10">
	                                            <div class="radio radio-info radio-inline">
	                                                <input type="radio" value="1" name="has_submenu"<c:if test="${menu.has_submenu==1}"> checked="checked"</c:if>>
	                                                <label for="inlineRadio1">&nbsp是</label>
	                                            </div>
	                                            <div class="radio radio-inline">
	                                                <input type="radio" value="0" name="has_submenu"<c:if test="${menu.has_submenu==0}"> checked="checked"</c:if>>
	                                    ...
	                                    <div class="form-group"><label class="col-sm-2 control-label">是否显示：</label>
	                                        <div class="col-sm-10">
	                                            <div class="radio radio-info radio-inline">
	                                                <input type="radio" value="1" name="isshow"<c:if test="${menu.isshow==1}"> checked="checked"</c:if>>
	                                                <label for="inlineRadio1">&nbsp是</label>
	                                            </div>
	                                            <div class="radio radio-inline">
	                                                <input type="radio" value="0" name="isshow"<c:if test="${menu.isshow==0}"> checked="checked"</c:if>>
	                                                <label for="inlineRadio2">&nbsp否</label>
	                                            </div>
	                                        </div>
	                                    </div>                                    							
