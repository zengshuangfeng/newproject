											...
											<div class="row">											
												<div class="col-sm-1" style="width:155px">
													<div class="form-group">
														<label class="control-label" >广告位置</label> <select
															name="style" class="form-control"
															onchange="$('#search_form').submit()">
															<option value="0"<c:if test="${style==0}"> selected="selected"</c:if>>广场banner</option>
															<option value="2"<c:if test="${style==2}"> selected="selected"</c:if>>房间广告</option>
															<option value="4"<c:if test="${style==4}"> selected="selected"</c:if>>用户信息</option>
														</select>
													</div>
												</div>
												<div class="col-sm-2">
													...
													<table
														...
														<thead>
															<tr>
																...
																<th class="text-right footable-last-column" style="width:90px">操作</th>
															</tr>
														</thead>
														<tbody>
															<c:forEach items="${recommendList}" var="recommend">
																<tr class="text-center">
																	...
																	<td class="col-xs-1">
                                   			 							<img class="col-xs-12" src="${uploadUrl}${recommend.pic}">
                           											</td>
																	<td><c:choose>
																			<c:when test="${not empty recommend.url and fn:contains(recommend.url,'birthday')}">生日卡片</c:when>
																	...
																	<td class="text-right footable-last-column"><a
																		...
																		href="${BASE_PATH}/recommend${name}/edit?id=${recommend.id}${query}">编辑</a>
																		<a class="btn btn-xs btn-outline btn-danger"
																		href="javascript:deleteRecommend(${recommend.id})">删除</a><br></td>
																</tr>
															</c:forEach>
														</tbody>
