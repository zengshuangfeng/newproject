								...
								<div class="panel-body">
									<fieldset class="form-horizontal">
										<form id="tab" action="${BASE_PATH}/h5/save"
											method="post">		
											<input type="hidden" name="id" value="${h5.id}"/>									
											<div class="form-group">
												<label class="col-sm-2 control-label">标题：</label>
												<div class="col-sm-5">
													<input type="text" name="title" class="form-control" value="${h5.title}">
												</div>
											</div>	
											<div class="form-group">
												<label class="col-sm-2 control-label">来源：</label>
												<div class="col-sm-5">
													<input type="text" name="author" class="form-control" value="${h5.author}">
												</div>
											</div>	
											<div class="form-group">
												...
												<div class="btn-group col-sm-6">
													<div class="fl">
														<input name="upload" id="fileInput" type="file" /><input
															type="hidden" id="imageHidden" name="banner"  value="${h5.banner}"/>
														<div id="singleDiv"></div>
														<img src="${uploadUrl}/${h5.banner}" class="wd200 mt10" id="image" />
													</div>
												</div>
											</div>
											<c:forEach items="${h5ContentList}" var="h5Content" varStatus="status">
											<c:choose>
											<c:when test="${h5Content.type==0}">
											<div class="form-group content-group">
												<label class="col-sm-2 control-label">段落：</label>
												<div class="col-sm-5">
													<textarea rows="3" class="form-control" name="remark${status.index}">${h5Content.content}</textarea>
												</div>
												<div class="col-sm-1"><a class="btn btn-xs btn-outline btn-danger" style="margin-top:20px"  href="javascript:void(0)" onclick="deleteTag(this)">删除</a></div>
											</div>	
											</c:when>
											<c:otherwise>
											<div class="form-group content-group">
												...
												<div class="btn-group col-sm-6">
													<div class="fl">
														<input name="upload" id="fileInput${status.index}" type="file" /><a class="btn btn-xs btn-outline btn-danger" style="margin-left:10px" href="javascript:void(0)" onclick="deleteTag(this)">删除</a><input
															type="hidden" id="picHidden${status.index}" name="pic${status.index}" value="${h5Content.content}" />
														<div id="singleDiv${status.index}"></div>
														<img src="${uploadUrl}/${h5Content.content}" class="wd200 mt10" id="pic${status.index}" />
													</div>
												</div>
											</div>
											</c:otherwise>
...
<script type="text/javascript">
	...
	$(function(){	
		...
		for(var i in arr)
		{
			singleImage("#fileInput"+arr[i], "peipei", "#pic"+arr[i], "input[name=pic"+arr[i]+"]", "singleDiv"+arr[i]);
		}
	});
