		...
		<div class="row wrapper border-bottom white-bg page-heading">
			<div class="col-lg-10">
				...
				<ol class="breadcrumb">
					...
					<li><a>社区</a></li>
					<li class="active"><strong>照片管理</strong></li>
				</ol>
			</div>
		</div>
		<div class="wrapper wrapper-content animated fadeInRight ecommerce">
			<c:set value="&uid=${uid}&start_time=${start_time}&end_time=${end_time}&title=${title}" var="query" />
			<div class="row">
				<div class="col-lg-12">
						...
						<div class="tab-content">
							<div id="tab-1" class="tab-pane active">
								<div class="panel-body">
									<div class="m-b-sm">
										<form action="${BASE_PATH}/community/picmanager" autocomplete="off"
												...
												<div class="col-sm-2">
													<div class="form-group">
														<label class="control-label" for="status">&nbsp;</label> <a
															href="${BASE_PATH}/community/picmanager/add"
															class="input-group">
														...
														<tbody>
															<c:forEach items="${postList}" var="postList">
																<tr class="text-center">
																	<td>${postList.id}</td>
																	<td class="col-xs-3" >
																		<c:if test="${not empty postList.post_pic}">
																			<c:set value="${fn:split(postList.post_pic, ',') }" var="p_pics" />
																			<c:forEach items="${p_pics}" var="p_pic" varStatus="status">
                                   			 								<img class="col-xs-4${status.index>2?' mt10':'' }"${status.index==3?' style="clear:both;"':'' } src="${uploadUrl}${p_pic}?imageView2/2/w/100" onclick="showImg('${uploadUrl}${p_pic}')">
                                   			 								</c:forEach>
                                   			 							</c:if>
																	</td>
																	<td>${postList.nickname}</td>
																	<td class="col-xs-2" <c:if test="${postList.title.length() >15}">style="text-align:left"</c:if>>${postList.title }</td>
                           											<td><c:if test="${postList.is_online==0 && postList.is_del==0}">
																		<span class="label label-danger">下线</span>
																	</c:if> <c:if test="${postList.is_online==1 && postList.is_del==0}">
																		<span class="label label-primary">上线</span>
																	</c:if>
																		 <c:if test="${postList.is_del==1}">
                           											...
                           											<td>
                           											<fmt:formatDate value="${postList.create_time}"  pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate> 
                           											</td>
                           											<td>
                           											<fmt:formatDate value="${postList.update_time}"  pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate> 
                           											</td>
                           											<td>
                           											<c:if test="${postList.is_top==0}">
																	...
																	<td><span class="actionclick"
																		...
																		<div class="actiondiv">
																			<a class="btn btn-xs btn-outline btn-success"
																				href="${BASE_PATH}/community/picmanager/edit?id=${postList.id}">编辑</a><br />
																			<a
																				href="${BASE_PATH}/community/picmanager/comment?post_id=${postList.id}"
																			...
																			<a
																		class="btn btn-xs btn-outline btn-success mt10"
																		href="javascript:TopPost(${postList.id},${postList.is_top})">
																		<c:if test="${postList.is_top==1}"><font color="red">取消置顶</font></c:if>
																		<c:if test="${postList.is_top==0}">置顶</c:if>																	
																		</a><br />	
...
<script type="text/javascript">
	...
	function deletePost(id) {
		deleteConfirm(id, "${BASE_PATH}/community/picmanager/postdelete", "该信息");
	}
	function Online(id) {
		swal({ 
			  title: "", 
			...
			function(isConfirm){ 
				...
				$.ajax({
					type : "POST",
					url : "${BASE_PATH}/community/picmanager/postonline",
					data : {
						id : id,				
					},
	...
	function Downline(id) {
		swal({ 
			...
			function(isConfirm){ 
				...
				$.ajax({
					type : "POST",
					url : "${BASE_PATH}/community/picmanager/postdownline",
					data : {
						id : id,				
					},
					success : function(data) {
