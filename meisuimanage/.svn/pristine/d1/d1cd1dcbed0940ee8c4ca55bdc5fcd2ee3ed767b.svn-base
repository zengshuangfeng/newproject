...
<div id="wrapper">
	...
	<div id="page-wrapper" class="gray-bg">
		...
		<c:set value="&uid=${uid}&date=${date}&type=${type}" var="query" />
			<div class="row">
				<div class="col-lg-12">
							...
							<div id="tab-1" class="tab-pane active">
								<div class="panel-body">
									...
									<ul class="nav nav-tabs">
										<li class="active"><a href="#">魅力值流水</a></li>
										<%-- <li><a href="${BASE_PATH}/useranchor/withdraw/waterlist?uid=${uid}">提现记录</a></li>	 --%>						
									</ul>	
									<div class="m-b-sm mt10">
										<div class="row">
											<div class="col-sm-6">
												<form action="${BASE_PATH}/useranchor/profitlist"
													autocomplete="off" method="get" class="form-inline" id="profit-form">
													<div class="form-group">
														<select name="type" class="form-control"
															onchange="$('#profit-form').submit()">
															<option value="-1">全部类型</option>
															<option value="0"<c:if test="${type==0}"> selected="selected"</c:if>>收礼</option>
															<option value="4"<c:if test="${type==4}"> selected="selected"</c:if>>奖励收益</option>
															<option value="5"<c:if test="${type==5}"> selected="selected"</c:if>>追回收益</option>
															<option value="6"<c:if test="${type==6}"> selected="selected"</c:if>>提现</option>
														</select>
													</div>	
													<div class="form-group">
													...
													<table
														...
														<thead>
															<tr>
																...
																<th class="text-center">魅力值变更</th>
																<th class="text-center">魅力值余额</th>
														...
														<tbody>
															<c:forEach items="${profitList}" var="profit">
																<tr class="text-center">
                           											<td>
                           												<c:choose>
                           													<c:when test="${profit.type==0}">收礼</c:when>
                           													<c:when test="${profit.type==4}">奖励收益</c:when>
                           													<c:when test="${profit.type==5}">收回收益</c:when>
                           													<c:when test="${profit.type==6}">提现</c:when>
                           												</c:choose>
                           											</td>
                           											<td>${profit.type==5||profit.type==6?"-":"+"}${profit.gift_virtual}</td>
                           											<td></td>
